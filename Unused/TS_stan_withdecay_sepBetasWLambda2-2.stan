...
parameters {
  ...
  #lambda population level (hyperprior)
	real<lower=0,upper=1> lm;
	real<lower=0> lss;
  ...
  # group level model (parameters per subject)
	...
	for (s in 1:NS) {
      ...
      for (t in 1:NT[s]) {
        ...
        ## update first state using lambda: (old qt1 value + (reward - value of second state)*alpha*lambda)
        qt1[c1[s,t]+1] = qt1[c1[s,t]+1] + alpha1[s]*(delta_1+delta_2* lambda[s]);
        ...
        # decaying non chosen to make more like regression model
        nc1 <- 2 - c1[s,t];
        nst <- 3 - st[s,t];
        nc2 <- 2 - c2[s,t];
        
        qt1[nc1]<- qt1[nc1] * (1-(alpha1[s]));
        qt2[st[s,t], nc2] <- qt2[st[s,t],nc2] * (1-(alpha1[s]));
        qt2[nst,1] <- qt2[nst,1] * (1-(alpha1[s]));
        qt2[nst,2] <- qt2[nst,2] * (1-(alpha1[s]));
